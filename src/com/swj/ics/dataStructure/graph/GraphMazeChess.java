...
public class GraphMazeChess {
   ...
   * 2、计算下一个需要移动的位置的时候，不是由弹出的 节点位置算起，而是用 currentPosition 算起，currentPosition 代表了当前位置的下一个位置，当前位置变换，那么 currentPosition 必须跟着变化
   * 3、辅助栈弹出数据的时机不对，当一个节点发现 4 个方向都是无法通行的时候，需要立即将之标注为失效节点，然后从栈中弹出下一个元素进行判断，而不是先弹出再判断，这样就是判断的下一个节点。
   * @param startPos
   * @param endPos
   * @return
   */
