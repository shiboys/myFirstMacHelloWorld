...
public class BinaryHeapMin<Key extends Comparable<Key>> implements Iterable<Key> {
  ...
  public void insert(Key key) {
    ...
    swim(size);
    if (!isMinHeap()) {
      throw new RuntimeException("heap is not the min heap right now.");
    }
  }
  ...
  int right(int k) {
    // 这个地方最好不要装逼，这里最好用 n*2+1 ,位移没弄好，优先级运算失败
    return (k << 1) + 1;
  }
  ...
  void resize(int newSize) {
    ...
    // 使用 System.arrayCopy 这种泛型的数组拷贝不过去，坑爹
    // 另外 二叉堆的使用一般都是从 数据的 1 开始
    System.arraycopy(pqArray, 1, newArr, 1, size);
    pqArray = newArr;
  }
  ...
  public static void main(String[] args) {
    BinaryHeapMin<Double> minHeap = new BinaryHeapMin<>();
