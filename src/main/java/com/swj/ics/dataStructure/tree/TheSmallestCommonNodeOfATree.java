...
public class TheSmallestCommonNodeOfATree {
   ...
   * 这样就能在树中从上到下找到第一个在两个节点之间的输入值
   * 2、如果这棵树不是二叉搜索树，甚至连二叉树都不是，只是普通的树，又该怎么办呢？
   * 解答：
   * 如果树中的每个节点（根节点除外）都有一个指向父节点的指针，那么这个问题可以转换成求两个链表的第一个公共节点。
   * 画图参考 《带父节点指针的树.png》
   ...
   * 3.1.6 此时遍历 G 节点，和 F 节点一样，这条路径不能到达 H 节点，遍历完 G 节点后，路径仍然是 A->B->D
   ...
   * 这里的解法太复杂了，参考我 leetcode 项目的解法，高效易懂。
   */
  ...
  static boolean getNodePathByInputNode(TreeNode inputNode, TreeNode rootNode, Deque<TreeNode> nodeList) {
    // 加入路径的是 rootNode ，如果写成 inputNode ，就大错特错了
  ...
  public static void main(String[] args) {
    ...
    TreeNode firstShareNode = getLowestCommonAncestorSimply(a, h, f);
    System.out.println(firstShareNode == null ? "null" : firstShareNode.value);
  }


  /**
   * 根据 阿东以及我在 leetcode 项目的算法，重写最小公共祖先的算法，简化和易懂, 将二叉树版本的改造为多叉树版本的
   * 同样是采用后序遍历的方式。
   * @param root
   * @param p1
   * @param p2
   * @return
   */
  static TreeNode getLowestCommonAncestorSimply(TreeNode root, TreeNode p1, TreeNode p2) {
    return findLCA(root,p1.value, p2.value);
  }

  static TreeNode findLCA(TreeNode root, String val1, String val2) {
    if (root == null) {
      return null;
    }
    if (Objects.equals(root.value, val1) || Objects.equals(val2, root.value)) {
      return root;
    }
    ...
    for (TreeNode child : root.nodeList) {
      nodeList.add(findLCA(child, val1, val2));
    }
    int nodeCount = (int) nodeList.stream().filter(Objects::nonNull).count();
    if (nodeCount >= 2) {
      return root; // 找到最小公共子节点
    } else if (nodeCount == 1) {
      return nodeList.stream().filter(Objects::nonNull).findFirst().get();
    } else {
      return null;
    }
  }
